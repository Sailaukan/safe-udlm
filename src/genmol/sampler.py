...
def load_model_from_path(path):
    ...
    model = SafeUDLM.load_from_checkpoint(path, map_location=device)
...
class Sampler:
    ...
    def _insert_mask(self, x, num_samples, min_add_len=18, mask_len=None, **kwargs):
        ...
        for _ in range(num_samples):
            if mask_len is None:
                ...
                add_seq_len = max(int(mask_len), 1)
    ...
    def fragment_completion(self, fragment, num_samples=1, apply_filter=True, softmax_temp=1.2, randomness=2,
                            gamma=0, min_add_len=18, mask_len=None, **kwargs):
