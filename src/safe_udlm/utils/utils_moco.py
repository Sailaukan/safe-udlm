...
class UniformTimeDistribution:
    """Minimal local replacement for BioNeMo's uniform time sampler."""
    ...
    def sample(
        ...
        if self.discrete_time:
            ...
            time_step = torch.randint(
                low=0,
                high=self.nsteps,
                size=(n_samples,) if isinstance(n_samples, int) else tuple(n_samples),
                device=device,
                generator=rng_generator,
            )
            return time_step

        time_step = torch.rand(
            n_samples,
            device=device,
            generator=rng_generator,
        )
        if self.min_t != 0.0 or self.max_t != 1.0:
