...
SAFE-UDLM is a SAFE-based molecule generation project built around uniform discrete diffusion language modeling (UDLM).
This repository is derived from the original [GenMol paper and codebase](https://arxiv.org/abs/2501.06158), but tracks the renamed SAFE-UDLM project and code.

Historical benchmark sections below may still mention GenMol where they refer to the original published results or checkpoints.
...
## Contribution
+ We introduce SAFE-UDLM, a model for unified and versatile molecule generation by building uniform discrete diffusion over SAFE molecular sequences.
...
+ We propose molecular context guidance (MCG), a guidance scheme for SAFE-UDLM to effectively utilize molecular context information.
+ We validate the efficacy and versatility of SAFE-UDLM on a wide range of drug discovery tasks.
...
(Optional) To train SAFE-UDLM from scratch, run the following command:
```bash
...
We suggest the use of [SAFE dataset V2](https://huggingface.co/datasets/datamol-io/safe-drugs) to train SAFE-UDLM (Note V2 removes some invalid molecules/corrupted SAFE strings. The original data that was used by GenMol is available here [SAFE dataset V1](https://huggingface.co/datasets/datamol-io/safe-gpt/tree/b83175cd7394).
...
If you see this error, it is likely coming from `/miniconda3/envs/safe-udlm/lib/python3.10/site-packages/tdc/chem_utils/oracle/oracle.py`, line 347, in readFragmentScores `_fscores = pickle.load(f)`
...
Extract the downloaded file into the `safe-udlm/oracle` directory.
</details>
...
```
The generated molecules will be saved in `scripts/exps/pmo/main/safe_udlm/results`.

Run the following command to evaluate the result:
```bash
python scripts/exps/pmo/eval.py ${file_name}
# e.g., python scripts/exps/pmo/eval.py scripts/exps/pmo/main/safe_udlm/results/albuterol_similarity_0.csv
```
